...
const handle_createDoc = (req, res, criteria) => {
    if (JSON.stringify(criteria) !== '{}' && criteria.name != "") {
        ...
        if(req.files.filetoupload){
            if (req.files.filetoupload.mimetype.startsWith("image/")) {
                criteria.photo = new Buffer.from(req.files.filetoupload.data).toString('base64');
            }
        }
...
const handle_UploadPhoto = (req, res, criteria) => {
        ...
        if (req.files.filetoupload.mimetype.startsWith("image/")) {
            //fs.readFile(req.files.filetoupload, (err,data) => {
               // assert.equal(err,null);
                updateDoc['photo'] = new Buffer.from(req.files.filetoupload.data).toString('base64');
                updateDocument(DOCID, updateDoc, (results) => {
                    console.log(results)
                    res.status(200).render('info', {message: `Updated ${results.result.nModified} document(s)`})
               // });
...
app.get('/createDoc', (req,res) => {
    handle_createDoc(req, res, req.query);
})

app.post('/createDoc', (req,res) => {
    handle_createDoc(req, res, req.body);
})
...
app.post('/update',(req,res) => {
   handle_Update(req, res,req.body);
}) 
...
app.get('/map', (req,res) => {
 ...
 app.post('/upload', (req,res) => {
     handle_UploadPhoto(req, res, req.body);
 })
